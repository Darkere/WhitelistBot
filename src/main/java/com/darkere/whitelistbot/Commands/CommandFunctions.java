...
public class CommandFunctions {
    ...
    static OptionData getServerChoice(Predicate<Server> select) {
        OptionData option = new OptionData(OptionType.STRING, "server", "Server to Select", true);
        ServerList.get().forEachServer(server -> {
            if (select == null || select.test(server)){
                option.addChoice(server.data.DisplayName, server.data.Name);
            }
        });
        return option;
    }
